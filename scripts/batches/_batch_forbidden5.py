...
WRONG_ALIASES = [
    ...
    ("阻性磁流体", "resistive-mhd", "zh", "forbidden", "误译resistive：正确为 电阻磁流体力学"),
    ...
    ("格拉德-沙弗拉诺夫方程", "grad-shafranov-equation", "zh", "forbidden", "误音译人名：正确保留 Grad-Shafranov方程"),

    # free-boundary equilibrium → 自由边界平衡
    ("自由边界均衡", "free-boundary-equilibrium", "zh", "forbidden", "误译equilibrium：正确为 自由边界平衡"),

    # force-free equilibrium → 无力平衡
    ("力自由平衡", "force-free-equilibrium", "zh", "forbidden", "误译force-free：正确为 无力平衡"),
    ("无外力平衡", "force-free-equilibrium", "zh", "forbidden", "误译force-free：正确为 无力平衡"),
    ...
    ("负三角形", "negative-triangularity", "zh", "forbidden", "误译triangularity：正确为 负三角形变"),
    ("负三角变形", "negative-triangularity", "zh", "forbidden", "误译triangularity：正确为 负三角形变"),
    ...
    ("q描述文件", "q-profile", "zh", "forbidden", "误译profile(计算机义)：正确为 安全因子剖面"),

    # rotation reversal → 旋转反转
    ("旋转逆转", "rotation-reversal", "zh", "deprecated", "非标准：应为 旋转反转"),
    ("旋转倒置", "rotation-reversal", "zh", "forbidden", "误译reversal：正确为 旋转反转"),
    ...
    ("漂移动力学", "drift-kinetics", "zh", "forbidden", "误译kinetics：正确为 漂移动理学"),
    ("漂移运动学", "drift-kinetics", "zh", "forbidden", "误译kinetics：正确为 漂移动理学"),

    # blob transport → 团块输运
    ("斑点传输", "blob-transport", "zh", "forbidden", "误译blob+transport：正确为 团块输运"),
    ("气泡输运", "blob-transport", "zh", "forbidden", "误译blob：正确为 团块输运"),

    # avalanche transport → 雪崩输运
    ("雪崩传输", "avalanche-transport", "zh", "forbidden", "误译transport：正确为 雪崩输运"),

    # staircase transport → 阶梯输运
    ("楼梯输运", "staircase-transport", "zh", "forbidden", "误译staircase：正确为 阶梯输运"),
    ("台阶输运", "staircase-transport", "zh", "deprecated", "非标准：应为 阶梯输运"),

    # turbulence suppression → 湍流抑制
    ("紊流压制", "turbulence-suppression", "zh", "forbidden", "双误(紊流+压制)：正确为 湍流抑制"),
    ("湍流压抑", "turbulence-suppression", "zh", "forbidden", "误译suppression：正确为 湍流抑制"),

    # turbulence saturation → 湍流饱和
    ("紊流饱和", "turbulence-saturation", "zh", "forbidden", "误译turbulence：正确为 湍流饱和"),

    # subcritical turbulence → 亚临界湍流
    ("次临界紊流", "subcritical-turbulence", "zh", "forbidden", "双误(次临界+紊流)：正确为 亚临界湍流"),
    ("亚临界紊流", "subcritical-turbulence", "zh", "forbidden", "误译turbulence：正确为 亚临界湍流"),

    # parallel transport → 平行输运
    ("平行传输", "parallel-transport", "zh", "forbidden", "误译transport：正确为 平行输运"),
    ("并行输运", "parallel-transport", "zh", "forbidden", "误译parallel(计算机义)：正确为 平行输运"),

    # critical gradient → 临界梯度
    ("关键梯度", "critical-gradient", "zh", "forbidden", "误译critical：正确为 临界梯度"),

    # critical balance → 临界平衡
    ("关键平衡", "critical-balance", "zh", "forbidden", "误译critical：正确为 临界平衡"),

    # particle confinement time → 粒子约束时间
    ("粒子限制时间", "particle-confinement-time", "zh", "forbidden", "误译confinement：正确为 粒子约束时间"),
    ("粒子封闭时间", "particle-confinement-time", "zh", "forbidden", "误译confinement：正确为 粒子约束时间"),

    # spectral transfer → 能谱级联
    ("频谱转移", "spectral-transfer", "zh", "forbidden", "误译spectral+transfer：正确为 能谱级联"),
    ("光谱传输", "spectral-transfer", "zh", "forbidden", "误译spectral+transfer：正确为 能谱级联"),

    # predator-prey oscillation → 捕食-被捕食振荡
    ("掠食者-猎物振荡", "predator-prey-oscillation", "zh", "forbidden", "误译：正确为 捕食-被捕食振荡"),
    ...
    ("等离子体面材料", "plasma-facing-material", "zh", "forbidden", "误译facing：正确为 面向等离子体材料"),
    ("面对等离子体材料", "plasma-facing-material", "zh", "forbidden", "误译facing：正确为 面向等离子体材料"),

    # plasma-surface interaction → 等离子体表面相互作用
    ("等离子体表面交互", "plasma-surface-interaction", "zh", "forbidden", "误译interaction：正确为 等离子体表面相互作用"),

    # detachment control → 脱靶控制
    ("分离控制", "detachment-control", "zh", "forbidden", "误译detachment：正确为 脱靶控制"),
    ("剥离控制", "detachment-control", "zh", "forbidden", "误译detachment：正确为 脱靶控制"),

    # detachment front → 脱靶前沿
    ("分离前沿", "detachment-front", "zh", "forbidden", "误译detachment：正确为 脱靶前沿"),
    ("脱离前锋", "detachment-front", "zh", "forbidden", "误译detachment+front：正确为 脱靶前沿"),
    ...
    ("中断侵蚀", "disruption-erosion", "zh", "forbidden", "误译disruption：正确为 破裂侵蚀"),
    ("破坏侵蚀", "disruption-erosion", "zh", "forbidden", "误译disruption：正确为 破裂侵蚀"),

    # physical sputtering → 物理溅射
    ("物理喷溅", "physical-sputtering", "zh", "forbidden", "误译sputtering：正确为 物理溅射"),
    ("物理飞溅", "physical-sputtering", "zh", "forbidden", "误译sputtering：正确为 物理溅射"),

    # prompt redeposition → 即时再沉积
    ("快速再沉积", "prompt-redeposition", "zh", "deprecated", "非标准：应为 即时再沉积"),
    ("及时再堆积", "prompt-redeposition", "zh", "forbidden", "误译prompt+redeposition：正确为 即时再沉积"),

    # fuel recycling → 燃料再循环
    ("燃料回收", "fuel-recycling", "zh", "forbidden", "误译recycling：正确为 燃料再循环"),
    ("燃料循环利用", "fuel-recycling", "zh", "forbidden", "误译recycling：正确为 燃料再循环"),

    # vapor shielding → 蒸汽屏蔽
    ("蒸气防护", "vapor-shielding", "zh", "forbidden", "误译shielding：正确为 蒸汽屏蔽"),
    ("蒸汽遮蔽", "vapor-shielding", "zh", "forbidden", "误译shielding：正确为 蒸汽屏蔽"),

    # long-leg divertor → 长腿偏滤器
    ("长支路偏滤器", "long-leg-divertor", "zh", "forbidden", "误译leg：正确为 长腿偏滤器"),
    ("长臂偏滤器", "long-leg-divertor", "zh", "forbidden", "误译leg：正确为 长腿偏滤器"),

    # pellet ablation → 弹丸烧蚀
    ("颗粒消融", "pellet-ablation", "zh", "forbidden", "误译pellet+ablation：正确为 弹丸烧蚀"),
    ...
    ("私通量区", "private-flux-region", "zh", "forbidden", "误译flux：正确为 私有磁通区"),
    ...
    ("大规模气体注射", "massive-gas-injection", "zh", "forbidden", "误译injection：正确为 大量气体注入"),
    ...
    ("隔热材料", "insulation-material", "zh", "forbidden", "误译insulation：正确为 绝缘材料"),

    # magnetic stored energy → 磁储能
    ("磁存储能量", "magnetic-stored-energy", "zh", "forbidden", "误译：正确为 磁储能"),

    # magnet structure → 磁体结构件
    ("磁铁结构", "magnet-structure", "zh", "forbidden", "误译magnet：正确为 磁体结构件"),
    ...
    ("环形场波纹", "tf-ripple", "zh", "forbidden", "双误(环形≠环向、波纹≠纹波)：正确为 环向场纹波"),
    ...
    ("中子磁通", "neutron-flux", "zh", "forbidden", "误译flux(电磁义)：正确为 中子通量"),

    # neutron spectrum → 中子能谱
    ("中子频谱", "neutron-spectrum", "zh", "forbidden", "误译spectrum：正确为 中子能谱"),
    ("中子光谱", "neutron-spectrum", "zh", "forbidden", "误译spectrum：正确为 中子能谱"),
    ...
    ("屏蔽渗透", "shielding-penetration", "zh", "forbidden", "误译penetration：正确为 屏蔽穿透"),

    # radiation streaming → 流道串流
    ("辐射流", "radiation-streaming", "zh", "forbidden", "误译streaming：正确为 流道串流"),
    ("辐射流动", "radiation-streaming", "zh", "forbidden", "误译streaming：正确为 流道串流"),
    ...
    ("方差减少", "variance-reduction", "zh", "forbidden", "误译reduction：正确为 方差缩减"),
    ("方差降低", "variance-reduction", "zh", "forbidden", "误译reduction：正确为 方差缩减"),

    # clearance index → 清洁解控指数
    ("清除指数", "clearance-index", "zh", "forbidden", "误译clearance：正确为 清洁解控指数"),
    ("间隙指标", "clearance-index", "zh", "forbidden", "误译clearance(机械义)：正确为 清洁解控指数"),

    # equivalent dose → 当量剂量
    ("等效剂量", "equivalent-dose", "zh", "forbidden", "误译equivalent：正确为 当量剂量"),

    # occupational exposure → 职业照射
    ("职业暴露", "occupational-exposure", "zh", "forbidden", "误译exposure：正确为 职业照射"),

    # gas production rate → 气体产额
    ("气体产生率", "gas-production-rate", "zh", "forbidden", "误译：正确为 气体产额"),
    ("气体生产率", "gas-production-rate", "zh", "forbidden", "误译production：正确为 气体产额"),

    # shielding labyrinth → 迷宫通道
    ("屏蔽迷宫", "shielding-labyrinth", "zh", "deprecated", "非标准：应为 迷宫通道"),

    # nuclear response function → 核响应函数
    ("核反应函数", "nuclear-response-function", "zh", "forbidden", "误译response≠reaction：正确为 核响应函数"),
    ...
    ("钨整体块", "tungsten-monoblock", "zh", "forbidden", "误译monoblock：正确为 钨单块"),

    # plasma spray coating → 等离子喷涂
    ("等离子体喷涂", "plasma-spray-coating", "zh", "forbidden", "误加'体'字：正确为 等离子喷涂"),
    ...
    ("金属氢化物层", "metal-hydride-bed", "zh", "forbidden", "误译bed：正确为 金属氢化物床"),

    # articulated boom → 铰接臂
    ("关节吊臂", "articulated-boom", "zh", "forbidden", "误译articulated：正确为 铰接臂"),
    ...
    ("回流功率", "recirculating-power", "zh", "forbidden", "误译recirculating：正确为 循环功率"),
    ...
    ("过夜资本成本", "overnight-capital-cost", "zh", "forbidden", "误译overnight：正确为 隔夜建设成本"),

    # vertical displacement event → 垂直位移事件
    ("竖直位移事件", "vertical-displacement-event", "zh", "deprecated", "非标准：应为 垂直位移事件"),

    # intermediate heat exchanger → 中间换热器
    ("中间热交换器", "intermediate-heat-exchanger", "zh", "deprecated", "非标准：应为 中间换热器"),

    # levelized cost → 平准化成本
    ("标准化成本", "levelized-cost", "zh", "forbidden", "误译levelized：正确为 平准化成本"),
    ("水平化成本", "levelized-cost", "zh", "forbidden", "误译levelized：正确为 平准化成本"),

    # engineering gain → 工程增益因子
    ("工程收益", "engineering-gain", "zh", "forbidden", "误译gain：正确为 工程增益因子"),

    # integrated commissioning → 综合调试
    ("集成调试", "integrated-commissioning", "zh", "deprecated", "非标准：应为 综合调试"),
    ("整体投运", "integrated-commissioning", "zh", "forbidden", "误译commissioning：正确为 综合调试"),

    # availability → 可用率
    ("可用性", "availability", "zh", "forbidden", "误译availability(堆工程应为比率)：正确为 可用率"),
    ...
    ("α通道", "alpha-channeling", "zh", "forbidden", "误译channeling：正确为 α粒子能量导引"),
    ("α引导", "alpha-channeling", "zh", "forbidden", "误译channeling：正确为 α粒子能量导引"),

    # aneutronic fusion → 无中子聚变
    ("非中子聚变", "aneutronic-fusion", "zh", "forbidden", "误译aneutronic：正确为 无中子聚变"),
    ("无中子融合", "aneutronic-fusion", "zh", "forbidden", "误用日语'融合'：正确为 无中子聚变"),

    # coaxial helicity injection → 同轴螺旋度注入
    ("同轴螺旋注入", "coaxial-helicity-injection", "zh", "forbidden", "缺字'度'：正确为 同轴螺旋度注入"),

    # spheromak → 球马克
    ("球状等离子体", "spheromak", "zh", "forbidden", "误译spheromak：正确为 球马克"),
    ("球形托马克", "spheromak", "zh", "forbidden", "spheromak≠spherical tokamak：正确为 球马克"),

    # direct energy conversion → 直接能量转换
    ("直接能量变换", "direct-energy-conversion", "zh", "deprecated", "非标准：应为 直接能量转换"),
    ("直接能源转化", "direct-energy-conversion", "zh", "forbidden", "误译：正确为 直接能量转换"),
]
