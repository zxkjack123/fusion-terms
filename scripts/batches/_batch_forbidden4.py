...
WRONG_ALIASES = [
    ...
    ("内部扭曲模", "internal-kink-mode", "zh", "forbidden", "误译internal：正确为 内扭曲模"),
    ("内部扭折模", "internal-kink-mode", "zh", "forbidden", "误译kink：正确为 内扭曲模"),
    ...
    ("互换不稳定性", "interchange-instability", "zh", "forbidden", "误译interchange：正确为 交换不稳定性"),
    ...
    ("倾覆不稳定性", "tilt-instability", "zh", "forbidden", "误译tilt：正确为 倾斜不稳定性"),
    ("翻转不稳定性", "tilt-instability", "zh", "forbidden", "误译tilt：正确为 倾斜不稳定性"),

    # micro-tearing mode → 微撕裂模
    ("微撕裂模式", "micro-tearing-mode", "zh", "deprecated", "非标准：应为 微撕裂模"),
    ("微断裂模", "micro-tearing-mode", "zh", "forbidden", "误译tearing：正确为 微撕裂模"),
    ...
    ("电子热梯度模", "etg-mode", "zh", "forbidden", "误译temperature：正确为 电子温度梯度模"),
    ...
    ("新经典传输", "neoclassical-transport", "zh", "forbidden", "误译transport：正确为 新经典输运"),

    # anomalous transport → 反常输运
    # (异常输运、异常传输 already in registry)

    # momentum transport → 动量输运
    ("动量传输", "momentum-transport", "zh", "forbidden", "误译transport：正确为 动量输运"),
    ("动量转移", "momentum-transport", "zh", "forbidden", "误译transport：正确为 动量输运"),

    # bootstrap current → 自举电流
    ("自引导电流", "bootstrap-current", "zh", "forbidden", "误译bootstrap：正确为 自举电流"),
    ("自持电流", "bootstrap-current", "zh", "forbidden", "误译bootstrap：应为 自举电流"),
    ...
    ("岁差漂移", "precession-drift", "zh", "forbidden", "误译precession：正确为 进动漂移"),
    ("旋进漂移", "precession-drift", "zh", "deprecated", "非标准：应为 进动漂移"),

    # NTV → 新经典环向粘滞
    ("新古典环向粘滞", "ntv", "zh", "forbidden", "误译neoclassical：正确为 新经典环向粘滞"),
    ...
    ("双液体模型", "two-fluid-model", "zh", "forbidden", "误译fluid：正确为 双流体模型"),
    ...
    ("电子环行辐射", "electron-cyclotron-emission", "zh", "forbidden", "误译cyclotron：正确为 电子回旋辐射"),
    ("电子回旋发射", "electron-cyclotron-emission", "zh", "forbidden", "误译emission：正确为 电子回旋辐射"),

    # recombination radiation → 复合辐射
    ("再结合辐射", "recombination-radiation", "zh", "forbidden", "误译recombination：正确为 复合辐射"),
    ("重组辐射", "recombination-radiation", "zh", "forbidden", "误译recombination：正确为 复合辐射"),

    # Thomson scattering → 汤姆逊散射
    ("托马斯散射", "thomson-scattering", "zh", "forbidden", "音译错：正确为 汤姆逊散射"),
    ("汤普逊散射", "thomson-scattering", "zh", "forbidden", "音译错(Thompson≠Thomson)：正确为 汤姆逊散射"),

    # charge-exchange recombination spectroscopy → 电荷交换复合光谱
    ("电荷交换光谱", "charge-exchange-recombination-spectroscopy", "zh", "deprecated", "缺recombination：应为 电荷交换复合光谱"),
    ("电荷转移复合光谱", "charge-exchange-recombination-spectroscopy", "zh", "forbidden", "误译exchange：正确为 电荷交换复合光谱"),

    # motional Stark effect → 运动斯塔克效应
    ("运动斯塔克效果", "motional-stark-effect", "zh", "forbidden", "误译effect：正确为 运动斯塔克效应"),

    # reflectometry → 反射仪
    ("反射测量法", "reflectometry", "zh", "forbidden", "误译：正确为 反射仪"),

    # far-infrared polarimetry → 远红外偏振仪
    ("远红外极化仪", "far-infrared-polarimetry", "zh", "forbidden", "误译polarimetry：正确为 远红外偏振仪"),
    ("远红外偏振测量", "far-infrared-polarimetry", "zh", "deprecated", "非标准：应为 远红外偏振仪"),

    # lower hybrid wave → 低杂波
    ("低混杂波", "lower-hybrid-wave", "zh", "forbidden", "误译hybrid：正确为 低杂波"),
    ("下混合波", "lower-hybrid-wave", "zh", "forbidden", "误译lower+hybrid：正确为 低杂波"),
    ...
    ("热液压学", "thermal-hydraulics", "zh", "forbidden", "误译hydraulics：正确为 热工水力"),
    ...
    ("远程维修", "remote-maintenance", "zh", "forbidden", "误译maintenance：正确为 遥维护"),
    ...
    ("分流器暗匣", "divertor-cassette", "zh", "forbidden", "误译divertor+cassette：正确为 偏滤器卡匣"),
    ...
    ("中子倍增器", "neutron-multiplier", "zh", "forbidden", "误译multiplier：正确为 中子倍增剂"),
    ("中子增殖器", "neutron-multiplier", "zh", "forbidden", "误译multiplier：正确为 中子倍增剂"),

    # tritium breeding ratio → 氚增殖比
    ("氚繁殖比", "tritium-breeding-ratio", "zh", "forbidden", "误译breeding：正确为 氚增殖比"),
    ("氚培育比", "tritium-breeding-ratio", "zh", "forbidden", "误译breeding：正确为 氚增殖比"),

    # neutron wall loading → 中子壁负荷
    ("中子墙负荷", "neutron-wall-loading", "zh", "forbidden", "误译wall：正确为 中子壁负荷"),
    ...
    ("照射脆化", "irradiation-embrittlement", "zh", "deprecated", "非标准：应为 辐照脆化"),
    ...
    # dust inventory → 粉尘存量
    ("灰尘库存", "dust-inventory", "zh", "forbidden", "误译dust+inventory：正确为 粉尘存量"),
    ("粉尘清单", "dust-inventory", "zh", "forbidden", "误译inventory：正确为 粉尘存量"),

    # tritium permeation barrier → 氚渗透阻挡层
    ("氚渗透阻碍层", "tritium-permeation-barrier", "zh", "forbidden", "用字错：正确为 氚渗透阻挡层"),
    ...
    ("培育包层", "breeding-blanket", "zh", "forbidden", "误译breeding：正确为 产氚包层"),
    ...
    ("惯性限制聚变", "inertial-confinement-fusion", "zh", "forbidden", "误译confinement：正确为 惯性约束聚变"),
    ("惯性封闭聚变", "inertial-confinement-fusion", "zh", "forbidden", "误译confinement：正确为 惯性约束聚变"),

    # thermonuclear fusion → 热核聚变
    ("热核融合", "thermonuclear-fusion", "zh", "forbidden", "误用日语'融合'：正确为 热核聚变"),
    ...
    ("球状托卡马克", "spherical-tokamak", "zh", "deprecated", "非标准：应为 球形托卡马克"),
    ("球型托克马克", "spherical-tokamak", "zh", "forbidden", "双误(型≠形、音译)：正确为 球形托卡马克"),

    # fusion reactivity → 聚变反应率
    ("聚变反应性", "fusion-reactivity", "zh", "forbidden", "误译reactivity：正确为 聚变反应率"),
    ("聚变反应活性", "fusion-reactivity", "zh", "forbidden", "误译reactivity：正确为 聚变反应率"),

    # technology readiness level → 技术就绪度
    ("技术成熟度", "technology-readiness-level", "zh", "deprecated", "非标准变体：应为 技术就绪度"),
    ("技术准备水平", "technology-readiness-level", "zh", "forbidden", "误译：正确为 技术就绪度"),
    ("技术就绪水平", "technology-readiness-level", "zh", "forbidden", "误译level：正确为 技术就绪度"),

    # Alfvén eigenmode → 阿尔芬本征模
    ("阿尔芬特征模", "alfven-eigenmode", "zh", "forbidden", "误译eigen-：正确为 阿尔芬本征模"),

    # toroidal Alfvén eigenmode → 环向阿尔芬本征模
    ("环形阿尔芬本征模", "toroidal-alfven-eigenmode", "zh", "deprecated", "非标准：应为 环向阿尔芬本征模"),
]
