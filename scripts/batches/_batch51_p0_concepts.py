...
concepts: list[tuple] = [
    ...
    ("gyro-bohm-scaling",   "concept",    "旋回Bohm标度",   "gyro-Bohm scaling",   "",    "active", "Turbulent transport scaling stronger than Bohm (gyro-radius dependent)"),
    ("turbulent-diffusion", "concept",    "湍流扩散输运",   "turbulent diffusion",  "",    "active", "Enhanced particle/heat diffusion due to plasma turbulence"),
    ("particle-pinch",      "concept",    "粒子箍缩",       "particle pinch",       "",    "active", "Inward convective particle transport in tokamak plasmas"),
    ("thermodiffusion",     "concept",    "热扩散",         "thermodiffusion",      "",    "active", "Temperature-gradient-driven particle flux (Soret effect in plasmas)"),

    ("# ==== Batch 51 P0: MHD stability ====",),
    ("external-kink",                "concept", "外扭曲模",       "external kink mode",                "",     "active", "Current-driven kink instability outside the plasma boundary"),
    ("sausage-instability",          "concept", "腊肠不稳定性",   "sausage instability",               "",     "active", "m=0 axisymmetric MHD instability in a Z-pinch"),
    ("pressure-driven-mode",         "concept", "压力驱动模",     "pressure-driven mode",              "",     "active", "MHD mode driven by pressure gradient (e.g. ballooning)"),
    ("current-driven-mode",          "concept", "电流驱动模",     "current-driven mode",               "",     "active", "MHD mode driven by current density gradient (e.g. kink, tearing)"),
    ("ideal-mhd-stability",          "concept", "理想MHD稳定性",  "ideal MHD stability",               "",     "active", "MHD stability analysis ignoring resistive effects"),
    ("resistive-instability",        "concept", "电阻不稳定性",   "resistive instability",             "",     "active", "MHD instability requiring finite plasma resistivity (tearing, resistive kink)"),
    ("global-alfven-eigenmode",      "concept", "全局Alfvén本征模","global Alfvén eigenmode",          "GAE",  "active", "Shear-Alfvén gap eigenmode with global structure"),

    ("# ==== Batch 51 P0: SOL / divertor physics ====",),
    ("parallel-heat-flux", "metric",   "平行热流",         "parallel heat flux",    "",    "active", "Heat flux density along magnetic field lines toward divertor"),
    ("filament",           "concept",  "等离子体丝状体",   "filament",              "",    "active", "Coherent field-aligned density structure in SOL (also called blob)"),
    ("divertor-leg",       "concept",  "偏滤器腿",         "divertor leg",          "",    "active", "Magnetic flux path from X-point to divertor target (inner/outer)"),
    ("flux-tube",          "concept",  "磁通管",           "flux tube",             "",    "active", "Tube-shaped region bounded by magnetic field lines"),
    ("connection-length",  "metric",   "连接长度",         "connection length",     "",    "active", "Field-line length from outboard midplane to divertor target"),
    ("wetted-area",        "metric",   "湿润面积",         "wetted area",           "",    "active", "Divertor target area receiving significant heat flux"),

    ("# ==== Batch 51 P0: Fuel cycle ====",),
    ("fuel-cycle",          "concept",  "燃料循环",     "fuel cycle",            "",    "active", "Closed loop of fuel supply, burn, exhaust, and reprocessing in a fusion plant"),
    ("isotope-separation",  "concept",  "同位素分离",   "isotope separation",    "",    "active", "Process to separate hydrogen isotopes (H, D, T)"),
    ("hydrogen-isotope",    "concept",  "氢同位素",     "hydrogen isotope",      "",    "active", "Isotopes of hydrogen: protium H, deuterium D, tritium T"),
    ("protium",             "concept",  "氕",           "protium",               "H",   "active", "Lightest hydrogen isotope (mass number 1)"),
    ("tritium-storage",     "system",   "氚储存",       "tritium storage",       "",    "active", "Systems for safe containment and inventory of tritium"),
    ("tritium-recovery",    "concept",  "氚回收",       "tritium recovery",      "",    "active", "Extraction of tritium from blanket breeder or plasma exhaust"),
    ("tritium-processing",  "concept",  "氚处理",       "tritium processing",    "",    "active", "Purification, isotope separation, and preparation of tritium for re-fueling"),
    ("palladium-membrane",  "material", "钯膜",         "palladium membrane",    "",    "active", "Pd-based selective membrane for hydrogen isotope permeation/separation"),
    ("getter-bed",          "system",   "吸气剂床",     "getter bed",            "",    "active", "Metal getter (U, Zr-alloy) bed for reversible tritium storage"),
    ("glovebox",            "system",   "手套箱",       "glovebox",              "",    "active", "Sealed enclosure with inert atmosphere for tritium handling"),
]
...
aliases: list[tuple] = [
    ...
    ("gyro-Bohm transport scaling","gyro-bohm-scaling", "en",    "alias",     "expanded form"),
    ("旋回玻姆标度",               "gyro-bohm-scaling", "zh",    "alias",     "full-Chinese transliteration"),
    # turbulent-diffusion
    ("turbulent diffusion",    "turbulent-diffusion", "en", "preferred", ""),
    ("湍流扩散输运",           "turbulent-diffusion", "zh", "preferred", "preferred zh (distinguishes from 湍流扩展)"),
    ...
    ("temperature screening",  "thermodiffusion", "en", "alias",     "tokamak-specific term"),
    ...
    ("pressure driven instability","pressure-driven-mode", "en", "alias",     "instability form"),
    ...
    ("current driven instability","current-driven-mode", "en", "alias",     "instability form"),
    ...
    ("global Alfven eigenmode","global-alfven-eigenmode", "en",   "alias",     "no-accent form"),
    ("全局阿尔芬本征模",      "global-alfven-eigenmode", "zh",   "alias",     "full-Chinese transliteration"),
    ...
    ("internal kink instability","internal-kink-mode", "en", "alias", "instability form"),
...
evidence: list[tuple] = [
    ("# ==== Batch 51 P0 ====",),
    ("gyro-bohm-scaling",      "https://en.wikipedia.org/wiki/Bohm_diffusion",  "", "", TODAY),
    ("turbulent-diffusion",    "internal:fusion-transport:turbulent-diffusion",  "", "", TODAY),
    ...
    ("sausage-instability",    "https://en.wikipedia.org/wiki/Pinch_(plasma_physics)", "", "", TODAY),
    ...
    ("hydrogen-isotope",       "https://en.wikipedia.org/wiki/Isotopes_of_hydrogen", "", "", TODAY),
...
if __name__ == "__main__":
    ...
    print(f"✓ Appended {sum(1 for r in concepts if not r[0].startswith('#'))} concept rows")

    write_tsv_rows(REG / "aliases.tsv", aliases)
    print(f"✓ Appended {sum(1 for r in aliases if not r[0].startswith('#'))} alias rows")

    write_tsv_rows(REG / "evidence.tsv", evidence)
    print(f"✓ Appended {sum(1 for r in evidence if not r[0].startswith('#'))} evidence rows")
