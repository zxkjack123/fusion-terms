...
- 发布文档口径对齐：README 中示例 tag 从 `v2026.03.24.1` 更新为 `v2026.03.25.1`。
- 历史计划文档 `next-improvement-cycle.md` 增加归档状态说明，避免与已执行完成的 2026-03-25 计划混淆。
...
- CI 工作流 YAML 统一为 2-space 缩进风格，提升可读性并与仓库其余配置文件风格对齐（行为无变更）。
- 发布文档示例 tag 更新为 `v2026.03.24.1`，便于下游按最新版本执行构建/验收流程。
...
- **Batch 脚本归档**
  - 将 25 个 `_batch*.py` 脚本从仓库根目录迁移到 `scripts/batches/`，统一归档一次性批处理脚本。
