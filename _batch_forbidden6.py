...
WRONG_ALIASES = [
    ...
    ("混合场景", "hybrid-scenario", "zh", "forbidden", "误译scenario：正确为 混合运行模式"),
    ("混合方案", "hybrid-scenario", "zh", "forbidden", "误译scenario：正确为 混合运行模式"),

    # steady-state operation → 稳态运行
    ("稳定状态操作", "steady-state-operation", "zh", "forbidden", "误译：正确为 稳态运行"),

    # inductive operation → 感应运行
    ("电感运行", "inductive-operation", "zh", "forbidden", "误译inductive：正确为 感应运行"),
    ("归纳运行", "inductive-operation", "zh", "forbidden", "误译inductive：正确为 感应运行"),

    # non-inductive operation → 非感应运行
    ("非电感运行", "non-inductive-operation", "zh", "forbidden", "误译inductive：正确为 非感应运行"),

    # plasma confinement → 等离子体约束
    ("等离子体限制", "plasma-confinement", "zh", "forbidden", "误译confinement：正确为 等离子体约束"),
    ("等离子体封闭", "plasma-confinement", "zh", "forbidden", "误译confinement：正确为 等离子体约束"),

    # plasma equilibrium → 等离子体平衡
    ("等离子体均衡", "plasma-equilibrium", "zh", "forbidden", "误译equilibrium：正确为 等离子体平衡"),

    # plasma ramp-up → 等离子体升流
    ("等离子体升坡", "plasma-ramp-up", "zh", "forbidden", "误译ramp-up：正确为 等离子体升流"),
    ("等离子体斜升", "plasma-ramp-up", "zh", "forbidden", "误译ramp-up：正确为 等离子体升流"),

    # plasma ramp-down → 等离子体降流
    ("等离子体降坡", "plasma-ramp-down", "zh", "forbidden", "误译ramp-down：正确为 等离子体降流"),
    ("等离子体斜降", "plasma-ramp-down", "zh", "forbidden", "误译ramp-down：正确为 等离子体降流"),
    ...
    ("燃点", "ignition-temperature", "zh", "forbidden", "误译(化学义)：聚变应为 点火温度"),

    # extended MHD → 扩展MHD
    ("延伸MHD", "extended-mhd", "zh", "forbidden", "误译extended：正确为 扩展MHD"),

    # MHD stability limit → MHD稳定极限
    ("MHD稳定性限制", "mhd-stability-limit", "zh", "forbidden", "误译limit：正确为 MHD稳定极限"),

    # hot-ion mode → 热离子模式
    ("热离子模", "hot-ion-mode", "zh", "deprecated", "缺字'式'：应为 热离子模式"),

    # plasma rotation → 等离子体旋转
    ("等离子旋转", "plasma-rotation", "zh", "forbidden", "缺字'体'：正确为 等离子体旋转"),
    ("等离子体转动", "plasma-rotation", "zh", "deprecated", "非标准：应为 等离子体旋转"),
    ...
    ("电子伯恩斯坦波动", "electron-bernstein-wave", "zh", "forbidden", "误加'动'：正确为 电子伯恩斯坦波"),

    # ion Bernstein wave → 离子伯恩斯坦波
    ("离子伯恩斯坦波动", "ion-bernstein-wave", "zh", "forbidden", "误加'动'：正确为 离子伯恩斯坦波"),

    # nonlinear coupling → 非线性耦合
    ("非线性联接", "nonlinear-coupling", "zh", "forbidden", "误译coupling：正确为 非线性耦合"),
    ("非线性联结", "nonlinear-coupling", "zh", "forbidden", "误译coupling：正确为 非线性耦合"),
    ...
    # ray tracing → 射线追踪
    ("光线追踪", "ray-tracing", "zh", "forbidden", "误译ray(计算机图形学义)：正确为 射线追踪"),

    # collisional damping → 碰撞阻尼
    ("碰撞衰减", "collisional-damping", "zh", "forbidden", "误译damping：正确为 碰撞阻尼"),
    ("碰撞减振", "collisional-damping", "zh", "forbidden", "误译damping：正确为 碰撞阻尼"),

    # continuum kinetic → 连续谱动理学
    ("连续动力学", "continuum-kinetic", "zh", "forbidden", "误译kinetic：正确为 连续谱动理学"),
    ("连续谱运动学", "continuum-kinetic", "zh", "forbidden", "误译kinetic：正确为 连续谱动理学"),

    # Fokker-Planck equation → Fokker-Planck方程
    ("福克尔-普朗克方程", "fokker-planck-equation", "zh", "forbidden", "误音译人名：正确保留 Fokker-Planck方程"),

    # Vlasov equation → Vlasov方程
    ("弗拉索夫方程", "vlasov-equation", "zh", "forbidden", "误音译人名：正确保留 Vlasov方程"),

    # reaction rate coefficient → 反应速率系数
    ("反应率系数", "reaction-rate-coefficient", "zh", "deprecated", "缺字'速'：应为 反应速率系数"),
    ...
    ("粒子内胞法", "particle-in-cell", "zh", "forbidden", "误译in-cell：正确为 粒子网格法"),
    ...
    ("磁性自我组织", "magnetic-self-organization", "zh", "forbidden", "误译：正确为 磁自组织"),

    # magnetic nozzle → 磁喷管
    ("磁性喷嘴", "magnetic-nozzle", "zh", "forbidden", "误译nozzle：正确为 磁喷管"),

    # flux expansion → 磁通膨胀
    ("通量扩张", "flux-expansion", "zh", "forbidden", "误译flux+expansion：正确为 磁通膨胀"),
    ...
    ("磁通保守器", "flux-conserver", "zh", "forbidden", "误译conserver：正确为 磁通守恒器"),

    # flux confinement → 磁通约束
    ("通量限制", "flux-confinement", "zh", "forbidden", "误译flux+confinement：正确为 磁通约束"),

    # quasi-axisymmetry → 准轴对称
    ("拟轴对称", "quasi-axisymmetry", "zh", "deprecated", "非标准：应为 准轴对称"),

    # quasi-helical symmetry → 准螺旋对称
    ("拟螺旋对称", "quasi-helical-symmetry", "zh", "deprecated", "非标准：应为 准螺旋对称"),
    ...
    ("偏滤器障板", "divertor-baffle", "zh", "forbidden", "误译baffle：正确为 偏滤器挡板"),
    ("分流器挡板", "divertor-baffle", "zh", "forbidden", "误译divertor：正确为 偏滤器挡板"),

    # divertor heat flux → 偏滤器热负荷
    ("偏滤器热通量", "divertor-heat-flux", "zh", "forbidden", "误译heat flux：正确为 偏滤器热负荷"),

    # divertor replacement → 偏滤器更换
    ("偏滤器替换", "divertor-replacement", "zh", "deprecated", "非标准：应为 偏滤器更换"),

    # Super-X divertor → Super-X偏滤器
    ("超X偏滤器", "super-x-divertor", "zh", "forbidden", "误译Super：正确保留 Super-X偏滤器"),

    # liquid metal divertor → 液态金属偏滤器
    ("液体金属偏滤器", "liquid-metal-divertor", "zh", "forbidden", "误译liquid：正确为 液态金属偏滤器"),
    ...
    ("能量粒子", "energetic-particle", "zh", "forbidden", "误译energetic：正确为 高能粒子"),
    ...
    ("基底压力", "base-pressure", "zh", "forbidden", "误译base+pressure：正确为 本底气压"),
    ...
    ("压力底座", "pressure-pedestal", "zh", "forbidden", "误译pedestal：正确为 压力基座"),
    ("压力台座", "pressure-pedestal", "zh", "forbidden", "误译pedestal：正确为 压力基座"),

    # temperature pedestal → 温度基座
    ("温度底座", "temperature-pedestal", "zh", "forbidden", "误译pedestal：正确为 温度基座"),
    ...
    ("激活分析", "activation-analysis", "zh", "forbidden", "误译activation：正确为 活化分析"),
    ...
    ("中水平废物", "intermediate-level-waste", "zh", "forbidden", "误译level：正确为 中放废物"),
    ("中放射性废物", "intermediate-level-waste", "zh", "deprecated", "非标准：应为 中放废物"),
    ...
    ("废物分类", "waste-classification", "zh", "forbidden", "误译classification(分级≠分类)：正确为 废物分级"),

    # ALARA → 合理可行尽量低
    ("阿拉拉", "alara", "zh", "forbidden", "误音译缩写：正确展开为 合理可行尽量低"),

    # derived air concentration → 导出空气浓度
    ("衍生空气浓度", "derived-air-concentration", "zh", "forbidden", "误译derived：正确为 导出空气浓度"),

    # in-vessel LOCA → 真空室内失冷事故
    ("容器内LOCA", "in-vessel-loca", "zh", "forbidden", "误译vessel：正确为 真空室内失冷事故"),

    # ex-vessel LOCA → 真空室外失冷事故
    ("容器外LOCA", "ex-vessel-loca", "zh", "forbidden", "误译vessel：正确为 真空室外失冷事故"),
    ...
    ("液体金属冷却剂", "liquid-metal-coolant", "zh", "forbidden", "误译liquid：正确为 液态金属冷却剂"),

    # lithium coating → 锂涂覆
    ("锂涂层", "lithium-coating", "zh", "deprecated", "非标准(coating→涂覆≠涂层)：应为 锂涂覆"),
    ...
    ("功率沉积剖面", "power-deposition-profile", "zh", "deprecated", "非标准：应为 功率沉积分布"),

    # power degradation → 功率退化
    ("功率降解", "power-degradation", "zh", "forbidden", "误译degradation：正确为 功率退化"),

    # FRC merging → 场反并合
    ("FRC合并", "frc-merging", "zh", "forbidden", "误译merging：正确为 场反并合"),

    # spheromak merging → 球马克并合
    ("球马克合并", "spheromak-merging", "zh", "forbidden", "误译merging：正确为 球马克并合"),

    # compact toroid injection → 紧凑环注入
    ("紧凑环面注入", "compact-toroid-injection", "zh", "forbidden", "误译toroid(环≠环面)：正确为 紧凑环注入"),
    ...
    ("磁性诊断", "magnetic-diagnostics", "zh", "forbidden", "误译magnetic：正确为 磁诊断"),

    # impurity spectroscopy → 杂质光谱
    ("杂质光谱学", "impurity-spectroscopy", "zh", "deprecated", "非标准：应为 杂质光谱"),

    # integrated modeling → 集成建模
    ("集成模型", "integrated-modeling", "zh", "forbidden", "误译modeling(动词义)：正确为 集成建模"),
    ("一体化模型", "integrated-modeling", "zh", "forbidden", "误译：正确为 集成建模"),

    # surrogate model → 代理模型
    ("替代模型", "surrogate-model", "zh", "forbidden", "误译surrogate：正确为 代理模型"),
    ...
    ("减速时间", "slowing-down-time", "zh", "forbidden", "误译slowing-down：正确为 滑脱时间"),
    ...
    ("同步加速器辐射", "synchrotron-radiation", "zh", "forbidden", "啰嗦/误译：正确为 同步辐射"),

    # spin-polarized fuel → 自旋极化燃料
    ("自旋偏振燃料", "spin-polarized-fuel", "zh", "forbidden", "误译polarized：正确为 自旋极化燃料"),
]
