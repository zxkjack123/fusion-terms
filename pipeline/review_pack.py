...
def _parse_candidates_tsv(path: Path) -> tuple[str, dict[str, TsvRow]]:
    ...
    try:
        ...
        raise SystemExit(
            ...
            "Tip: candidates TSVs should be generated by pipeline.extract_candidates."
        )
    if not lines:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Exclude terms already curated in terms/allowlist_*.txt and terms/denylist.txt "
            "from the review pack (applies to both current and baseline so they don't show as removed)."
        ),
    )
    parser.add_argument(
        "--terms-dir",
        default="terms",
        help="Directory containing allow/deny/synonyms (used by --exclude-known-terms)",
    )
